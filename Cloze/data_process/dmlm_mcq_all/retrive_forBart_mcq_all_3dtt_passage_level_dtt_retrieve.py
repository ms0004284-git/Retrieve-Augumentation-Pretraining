...
def get_ans_dtt_pairs():
    with open(f'/user_data/Cloze/dataset/mcq/mcq_all_filter_sciq_overlap.json', "r", encoding="utf-8") as f:
        data = json.load(f)
        return data
...
for pair in ans_dtt_pairs:
    ...
    # 有些資料 distractors 數量不足三
    search_word = [pair['answer']]
    for dt in pair['distractors']:
        search_word.append(dt)
        ...
        for p in passages:
            try:
                training_data.append(
                        {
                            'sentence' : re.sub(fr'\b{search_word}\b', '<mask>', p), # count= 取代幾個 沒有放就是全部
...
with open('/user_data/Cloze/dataset/mcq/mask_sentence_with_dtt/bart/mcq_all_mask_passage_with_dtt_for_bart(3dtt)_dtt_retrieve.json', "w", encoding="utf-8") as f:

...
def get_ans_dtt_pairs():
    with open(f'/user_data/Cloze/dataset/sciq/mask_sentence_with_dtt/sciq_total_filter_mcq_overlap.json', "r", encoding="utf-8") as f:
        data = json.load(f)
        return data
    ...
    for p in passages:
        try:
            training_data.append(
                    {
                        'sentence' : re.sub(fr'\b{search_word}\b', '<extra_id_0>', p, count=1), # count= 取代幾個 沒有放就是全部
                        'label' : '<pad> <extra_id_0> {first_dtt} <extra_id_1> {second_dtt} <extra_id_2> {thrid_dtt} <extra_id_3>'
                        .format(
                                    first_dtt=pair['distractor'][0],
                                    second_dtt=pair['distractor'][1],
                                    thrid_dtt=pair['distractor'][2]
                                )
                    }
                )
        except:
...
with open('/user_data/Cloze/dataset/sciq_passsage_debug/sciq_mask(all)_passage_with_dtt_for_t5(3dtt)_filter.json', "w", encoding="utf-8") as f:
